...
function getStats(e){
	...
	var valor=obj.value;
	if(isNaN(valor)){
		var error='El valor del stat que acaba de introducir no es correcto.<br/>Debe ser numérico no una letra';
		setPopover(obj,error,"Error al introducir el valor");
		obj.value="";
	}else if(valor<1 || valor>99){
		var error='El valor del stat que acaba de introducir no es correcto.<br/>Debe contener un número entre: 1 y 99';
		setPopover(obj,error,"Error número 1-99",5000);
		obj.value="";
	}else{
		stats[parseInt(obj.getAttribute("id").split("_")[1])]=parseInt(obj.value);
	}
}
/**
...
function getPotencial(e){
	...
	potencial[parseInt(obj.getAttribute("id").split("_")[1])]=parseInt(e.target.value);
}
/**
	...
	switch(e.target.value){
		case "h":
		  if(contH<2){
		  	...
		  	var error='No puede seleccionar más habilidades';
		...
		case "c":
		  if(contC<2){
		  	...
		  	var error='No puede seleccionar más carencias';
...
function changehab_car(objeto,evento,mode){
	if(mode==1){
		hab_car[parseInt(objeto.getAttribute("id").split("_")[1])]=evento.target.value;
	}else{
		var temp=hab_car[parseInt(objeto.getAttribute("id").split("_")[1])];
		if(temp=="h"){
			contH--;
			hab_car[parseInt(objeto.getAttribute("id").split("_")[1])]=evento.target.value;
		}else{
...
function plusStats(e){
	var obj=e.target;
	var indice=parseInt(obj.getAttribute("id").split("_")[1]);
...
function setCalc(indice,posIni,power,pm,mode){
	if(mode===0){
		var temp=puntos[indice]+parseInt(PM[pm]*power);
		if(temp>7493)
			puntos[indice]=7493;
		else
			puntos[indice]+=parseInt(PM[pm]*power);
...
function changeStat(indice,posIni,mode){
	var objStats=document.getElementById("txt_"+indice);
	var objEntrenos=document.getElementById("entrenos_"+indice);
	var objPuntos=document.getElementById("puntos_"+indice);
	var posFinal=0;
	while(puntos[indice]>=PE[posFinal])
			posFinal++;
	if(mode===0){
		if(puntos[indice]<7493)
			entrenos[indice]++;
	...
	document.getElementById("totEntrenos").innerHTML=suma;
	document.getElementById("totTemporadas").innerHTML=parseInt(suma/36);
}

// getAltura: Obtenemos la edad y altura del jugador
function getAltura(){
	var objEdad=document.getElementById("txtEdad");
	edad=objEdad.value;
	var objAlturaIni=document.getElementById("txtAltura");
	alturaIni=objAlturaIni.value;

	// Comprobamos si la edad y altura son numeros
	if(isNaN(edad)){
		var error='La edad debe ser un número';
		setPopover(objEdad,error,"Error: edad letra",0,'left',false);
		objEdad.value="";
		return false;
	}
	else if(isNaN(alturaIni)){
		var error='La altura debe ser un número';
		setPopover(objAlturaIni,error,"Error: altura letra",0,"left",false);
		objAlturaIni.value="";
		return false;
	}
	else{
		// Si son numeros los convertimos y comprobamos sus valores
		edad=parseInt(edad);
		alturaIni=parseInt(alturaIni);
		if(edad<13 || edad>18){
			var error='La edad debe estar comprendida entre:<br/>13 y 18 años';
			setPopover(objEdad,error,"Límite edad erróneo",0,'left');
			objEdad.value="";
			return false;
		}
		if(alturaIni<165 || alturaIni>235){
			var error='La altura inicial debe estar comprendida entre:<br/>165 y 235 centímetros';
...
function setAltura(edad,alturaIni){
	var altMin=document.getElementById("txtAlturaMin");
	var altMax=document.getElementById("txtAlturaMax");
	var alturaMinima=0;
	var alturaMaxima=0;
