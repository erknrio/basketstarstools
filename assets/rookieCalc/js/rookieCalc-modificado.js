...
function minusStats(e) {
    'use strict';
	var obj = e.target;
	var indice = parseInt(obj.getAttribute("id").split("_")[1], 10);
	/* Obtenemos el indice para el potencial.
	* Este cambia segun el tipo de stat que se aumente
	* 0: Fisico
	* 1: Ataque
...
function changeStat(indice, posIni, mode) {
    'use strict';
	var objStats = document.getElementById("txt_"+indice), objEntrenos = document.getElementById("entrenos_"+indice), objPuntos = document.getElementById("puntos_"+indice), posFinal=0;
	while(puntos[indice ] >= PE[posFinal])
			posFinal++;
	if (mode === 0) {
		if (puntos[indice] < 7493)
			entrenos[indice]++;
		stats[indice] +=  parseInt(posFinal - posIni, 10);
	} else {
		if ((entrenos[indice]-1) < 0)
			entrenos[indice] = 0;
		else
			entrenos[indice]--;
		stats[indice] -= parseInt(posIni - posFinal, 10);
...
function getAltura() {
    'use strict';
	var objEdad = document.getElementById("txtEdad"), objAlturaIni = document.getElementById("txtAltura");
	...
	if (isNaN(edad)) {
		...
		alturaIni = parseInt(alturaIni, 10);
		if (edad < 13 || edad > 18) {
